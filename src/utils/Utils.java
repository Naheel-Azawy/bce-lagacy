...
public class Utils {
    ...
    public static int[] parseIntArray(String[] arr) {
        return parseIntArray(arr, 10);
    }

    public static int[] parseIntArray(String[] arr, int radix) {
        ...
        try {
            for (i = 0; i < arr.length; ++i) {
                if (arr[i].length() == 0)
                    continue;
                res[i] = Integer.parseInt(arr[i], radix);
            }
        } catch (Exception e) {
    ...
    public static String intToPrintableCharString(int c) {
